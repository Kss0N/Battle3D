...
object Logger:
    ...
    */
    enum Severity(colorCode: String):
    ...
    //Called first
    def init(path: String)  : Unit = myLogger = new Logger(path)

    def apply(path: String): Unit = init(path)
 ...
 * 
*/
class Logger private(val filePath: String = "battle3d.log"):
    import Logger.Severity

    private val file        = new FileOutputStream(filePath)
    ...
    */
    def newEntry(severity: Logger.Severity, msg: String): Unit = 
        ...
        try 
            ...
            bufWriter.write(msg, 0, msg.length())
            bufWriter.newLine()
        catch
            case e: IOException =>
